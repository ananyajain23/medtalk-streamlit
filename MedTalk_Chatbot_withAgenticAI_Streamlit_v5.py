...
# MedTalk is a Streamlit-based AI chatbot that allows users to upload medical books in PDF format and interactively query them 
# using natural language. It combines OCR, table parsing, and document chunking with advanced retrieval-augmented generation 
# (RAG) to answer complex medical questions.
...
# 5. Streamlit for UI:
# ✅ Users can upload multiple medical PDFs for automated text, table, and image (OCR) extraction.
...
st.set_page_config(page_title="MedTalk - Chatbot", page_icon="🩺")
st.title("🩺 MedTalk – Medical Book RAG Chatbot with AgenticAI 🛠️")
st.markdown("🔍 Upload your medical books and chat 💬 with them. Extracts **text**, **tables**, and **image text via OCR**.")
...
if st.session_state.get("vectorstore_built"):
    st.markdown("---")
    st.subheader("💬 Ask your questions regarding the Medical Book:")
...
# some questions asked regarding medical book: 

# create table of Risk of acquiring HIV infection by entry site, also tell me the page number of the pdf you're extracting table from
# create a table of Opioid analgesics indicating the drug, route of administration and duration of action. also tell me the page number of the pdf you're extracting table from.
# what is Alzheimer’s disease? can you explain in detail with bullet points.
# tell me the Symptoms Of Co-Alcohol Dependence in bullet points
# how does the close-up of a patient’s arm after allergy testing look?
# top of balding male's head image is on which page of the pdf i uploaded?
# A close-up view of a carbuncle on person’s back photograph is on which page number of the pdf i uploaded?

# create a table of recipients blood (which is further divided into abo antigens, abo antibodies and abo blood type) and its 
# reactions with the donor red blood cells (telling compatible and non compatible). also tell which page number of the pdf you're 
# extracting table from.
